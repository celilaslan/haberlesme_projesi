...
class DataBuffer::Impl {
public:
    Impl() : buffer_enabled_(false), recording_(false), replaying_(false),
             max_buffer_size_(100 * 1024 * 1024), current_buffer_size_(0) {} // 100MB default
    ...
    bool replayData(const std::string& filename, double speed_multiplier) {
        ...
        // Start replay thread
        replay_thread_ = std::thread([this, filename, speed_multiplier]() {
            performReplay(filename, speed_multiplier);
        });
...
bool DataBuffer::enableBuffering(size_t max_buffer_size_mb) {
    return pImpl->enableBuffering(max_buffer_size_mb);
}

bool DataBuffer::startRecording(const std::string& filename) {
    return pImpl->startRecording(filename);
}

bool DataBuffer::stopRecording() {
...
bool DataBuffer::isRecording() const {
    return pImpl->isRecording();
}

bool DataBuffer::isReplaying() const {
    return pImpl->isReplaying();
}

double DataBuffer::getBufferUsage() const {
    return pImpl->getBufferUsage();
}
