...
namespace TelemetryAPI {
       ...
       public:
        ...
        DataQuality getDataQuality(const std::string& uav_name, DataType type) {
            ...
            if (stats.total_packets > 0) {
                quality.packet_loss_rate =
                    static_cast<double>(stats.lost_packets) / (stats.total_packets + stats.lost_packets);
            } else {
        ...
        void recordDataReceived(const TelemetryData& data) {
            ...
            // Update bandwidth statistics
            {
                ...
                while (!bandwidth_samples_.empty() && bandwidth_samples_.front().timestamp < now - 3600000) {
    ...
    bool DataAnalyzer::enableDataValidation(bool enable) { return pImpl->enableDataValidation(enable); }

    std::vector<TelemetryData> DataAnalyzer::getHistoricalData(const std::string& uav_name, uint64_t start_time,
                                                               uint64_t end_time) {
    ...
    BandwidthStats DataAnalyzer::getBandwidthUsage() { return pImpl->getBandwidthUsage(); }
