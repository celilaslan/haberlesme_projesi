...
public:
    ...
    bool broadcastCommand(const std::vector<std::string>& uav_names,
        ...
        for (const auto& uav_name : uav_names) {
            ...
            if (it != fleet_status_.uav_statuses.end()) {
                ...
                it->second.last_command_status = all_successful ?
    ...
    FleetStatus getFleetStatus() {
        ...
        fleet_status_.overall_health_score = fleet_status_.total_uavs > 0 ?
            total_health / fleet_status_.total_uavs : 0.0;
...
private:
    ...
    double calculateUAVHealth(const UAVStatus& status, uint64_t time_since_last_seen) {
        ...
        double data_quality_factor = status.data_quality.data_freshness_score * 0.7 +
                                    (1.0 - status.data_quality.packet_loss_rate) * 0.3;
...
bool FleetManager::initialize(TelemetryClient* client) {
    return pImpl->initialize(client);
}

bool FleetManager::broadcastCommand(const std::vector<std::string>& uav_names,
                                   const std::string& command) {
    return pImpl->broadcastCommand(uav_names, command);
}

FleetStatus FleetManager::getFleetStatus() {
    return pImpl->getFleetStatus();
}
...
bool FleetManager::addUAV(const std::string& uav_name) {
    return pImpl->addUAV(uav_name);
}

bool FleetManager::removeUAV(const std::string& uav_name) {
    return pImpl->removeUAV(uav_name);
}
