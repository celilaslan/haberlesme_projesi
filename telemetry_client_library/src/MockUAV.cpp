...
namespace TelemetryAPI {
    ...
    class MockUAV::Impl {
       public:
        ...
        bool start() {
            ...
            simulation_thread_ = std::thread([this]() { runSimulation(); });
    ...
    bool MockUAV::simulateDataLoss(double loss_percentage) { return pImpl->simulateDataLoss(loss_percentage); }

    bool MockUAV::simulateLatency(int additional_ms) { return pImpl->simulateLatency(additional_ms); }

    bool MockUAV::injectTestData(const std::string& test_data) { return pImpl->injectTestData(test_data); }

    bool MockUAV::start() { return pImpl->start(); }

    bool MockUAV::stop() { return pImpl->stop(); }

    bool MockUAV::isRunning() const { return pImpl->isRunning(); }
