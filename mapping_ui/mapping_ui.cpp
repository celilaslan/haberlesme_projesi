 ...
 * @param udp_mapping_port Reference to store UDP mapping port
 * @return true if configuration loaded successfully, false otherwise
 */
bool loadConfig(int& tcp_publish_port, int& tcp_command_port, int& udp_mapping_port) {
    ...
    try {
        ...
        if (j.contains("ui_ports")) {
            ...
            udp_mapping_port = j["ui_ports"]["udp_mapping_port"];
            return true;
        } else {
 ...
 * 
 * Usage:
 * ./mapping_ui --protocol <tcp|udp> [--send UAV_NAME]
 * 
 * Required arguments:
...
int main(int argc, char* argv[]) {
    ...
    int tcp_publish_port, tcp_command_port, udp_mapping_port;
    loadConfig(tcp_publish_port, tcp_command_port, udp_mapping_port);
    ...
    if (protocol.empty()) {
        ...
        std::cerr << "Usage: ./mapping_ui --protocol <tcp|udp> [--send UAV_NAME]\n";
    ...
    if (protocol == "udp") {
        ...
        std::cout << "[Mapping UI] Using UDP protocol on port " << udp_mapping_port << " (telemetry reception only)\n";
        ...
        udp::socket socket(io_context, udp::endpoint(udp::v4(), udp_mapping_port));
        
        // Main UDP telemetry reception loop (read-only)
        std::cout << "Press Ctrl+C to stop the mapping UI..." << std::endl;
        while (g_running) {
        ...
        if (enableSender) {
            ...
            senderThread = std::thread([p = push.get(), target]() {
                std::string line;
                std::cout << "[Mapping UI] Type commands for " << target << " (press Enter to send, Ctrl+C to exit):" << std::endl;
            ...
            std::cout << "[Mapping UI] TCP Sender enabled to " << target << " via port " << tcp_command_port << "\n";
        }
    ...
    std::cout << "Mapping UI stopped." << std::endl;
    return 0;
}
